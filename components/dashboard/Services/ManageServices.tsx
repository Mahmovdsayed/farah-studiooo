...
const ManageServices = ({ services }: IProps) => {
    return <>
        <Card className="w-full mt-4 py-4 px-0">
            <CardHeader>
            ...
            <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">

                {
                    services?.map((service: servicesTypes) => (
                        <Card className="bg-secondary py-4 px-0" key={service._id}>
                            <CardHeader>
